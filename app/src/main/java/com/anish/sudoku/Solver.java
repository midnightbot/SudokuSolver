...
public class Solver {
    ...
    public void getEmptyBoxIndexes(){
        for(int r=0;r<9;r++)
        {
            ...
            {
                if(this.board[r][c]==0){
                    this.emptyBoxIndex.add(new ArrayList<Object>());
        ...
        if(this.board[row][col] >0){
            for(int i=0;i<9;i++){
                if(this.board[i][col] == this.board[row][col] && row!=i){
                    return false; //for checking rows (no same number in same row)
                }

                if(this.board[row][i] == this.board[row][col] && col!=i){
                    return false;// for checking columns
                }
            }
            //checking no same number in same box
            int boxRow = row/3;
            int boxCol = col/3;
